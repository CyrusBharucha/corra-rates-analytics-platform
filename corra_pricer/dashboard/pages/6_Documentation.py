...
- **No external pricing libraries.** Curve bootstrapping, discounting, risk, and scenario math are all written in Python, so every formula is traceable in the code.
...
telescopes to the single expression above.
...
live on the Market page's Bootstrap Diagnostics tab.
"""
    )
    st.write("")
...
All three can be compared directly on the Market page.
"""
    )
    st.write("")
...
with tab_risk:
    ...
    st.markdown(
        """
Every risk number is computed by shocking the curve and re-running the pricer — no closed-form Greeks. Each result is the NPV before and after a specific curve move.
...
as illustrative/stylized, not calibrated forecasts, and are distinct from Historical Replay,
which prices swaps on real dates with real Bank of Canada data.
...
- **Single-curve** framework: the same curve forecasts and discounts. A production multi-curve
  ...
  is out of scope: it would break the telescoping-identity floating-leg valuation and
  isn't needed for a single-currency, single-index CORRA OIS.
...
- Carry, roll-down, and theta (time-decay valuation) are **not implemented**, they require a
  ...
  curve's own forward-implied shape) than the bump-and-reprice approach used throughout.
...
- Macro scenario shock shapes (recession, inflation shock, etc.) are illustrative/stylized,
  not calibrated to any specific forecast or historical regression.
