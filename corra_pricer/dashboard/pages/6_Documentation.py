...
- **Log-linear on discount factor** -- equivalent to piecewise-flat instantaneous forward rates.
...
| **Convexity** | `NPV(+1bp) + NPV(-1bp) - 2*NPV(base)`, left in **unnormalized dollar terms** rather than divided by price -- a swap's NPV can sit at/near zero at its own fair rate, which would make a price-normalized convexity blow up. |
