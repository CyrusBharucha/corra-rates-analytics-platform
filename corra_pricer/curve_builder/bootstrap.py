...
For each bond node, we solve the discount factor that makes a par bond with
that coupon and maturity price to 100, given discount factors for earlier
coupon dates already known (or, when a coupon date falls in a gap between
bootstrapped nodes, from flat extrapolation off the last known node).
