...
class RiskEngine:
    """Bundles all risk computations for a (swap, curve) pair."""

    def __init__(self, swap, curve, bump_bp: float = 1.0):
        self.swap = swap
        self.curve = curve
        self.bump_bp = bump_bp

    def dv01(self) -> float:
        return compute_dv01(self.swap, self.curve, self.bump_bp)

    def pv01(self) -> float:
        return compute_pv01(self.swap, self.curve)

    def krd(self, buckets: list[float] | None = None) -> dict[str, float]:
        return compute_krd(self.swap, self.curve, buckets, self.bump_bp)

    def convexity(self) -> float:
        return compute_convexity(self.swap, self.curve, self.bump_bp)

    def dollar_duration(self) -> float:
        return compute_dollar_duration(self.swap, self.curve)

    def gamma_dv01(self) -> float:
        return compute_gamma_dv01(self.swap, self.curve)

    def leg_dv01(self) -> dict[str, float]:
        return compute_leg_dv01(self.swap, self.curve, self.bump_bp)

    def fixed_leg_macaulay_duration(self) -> float:
        return compute_fixed_leg_macaulay_duration(self.swap, self.curve)

    def extended_metrics(self) -> dict:
        return compute_extended_risk_metrics(self.swap, self.curve)

    def report(self, buckets: list[float] | None = None) -> RiskReport:
        return build_risk_report(self.swap, self.curve, buckets, self.bump_bp)
