...
class ScenarioEngine:
    """Runs named, custom, and Monte Carlo scenarios for a (swap, curve) pair."""

    def __init__(self, swap, curve, buckets: list[float] | None = None):
        self.swap = swap
        self.curve = curve
        self.buckets = buckets

    def run(self, scenario_name: str) -> ScenarioResult:
        return run_scenario(self.swap, self.curve, scenario_name, self.buckets)

    def run_custom(self, bucket_shocks_bp: dict, name: str = "custom") -> ScenarioResult:
        return run_custom_scenario(self.swap, self.curve, bucket_shocks_bp, name, self.buckets)

    def run_all(self) -> list[ScenarioResult]:
        return run_all_scenarios(self.swap, self.curve, self.buckets)

    def summary_table(self):
        return scenario_summary_table(self.swap, self.curve, self.buckets)

    def run_monte_carlo(
        self, n_simulations: int = 200, shock_std_bp: float = 25.0, seed: int | None = None,
    ):
        return run_monte_carlo_scenarios(
            self.swap, self.curve, n_simulations, shock_std_bp, self.buckets, seed,
        )
